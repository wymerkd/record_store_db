class Song
  ...
  def self.all
    self.get_songs('SELECT * FROM songs;')
  end

  def self.get_songs(db_query)
    returned_songs = DB.exec(db_query)
    songs = []
  ...
  def self.find(id)
    ...
    name = song.fetch('name')
    id = song.fetch('id').to_i
    Song.new({:name => name, :id => id, :album_id => album_id})
  end
  ...
  def self.find_by_album(alb_id)
    self.get_songs("SELECT * FROM songs WHERE album_id = #{alb_id};")
  end
